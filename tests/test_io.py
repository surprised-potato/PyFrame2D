...
def representative_model():
    """Creates a somewhat complex StructuralModel for save/load testing."""
    ...
    # Materials
    mat_steel = Material(10, "Steel S275", 205e9)
    mat_conc = Material(11, "Concrete C30", 33e9)
    model.add_material(mat_steel)
    model.add_material(mat_conc)

    # Sections
    sec_rect = RectangularProfile(101, "R 200x300", width=0.2, height=0.3)
    sec_sq = SquareProfile(102, "SHS 150x150", side_length=0.15)
    sec_ibeam = IBeamProfile(103, "IPE 200", height=0.2, flange_width=0.1,
                             flange_thickness=0.008, web_thickness=0.005)
    model.add_section(sec_rect)
    model.add_section(sec_sq)
    model.add_section(sec_ibeam)

    # Members
    mem1 = Member(201, n1, n2, mat_steel, sec_ibeam)
    mem2 = Member(202, n2, n3, mat_conc, sec_rect)
    mem3 = Member(203, n2, n4, mat_steel, sec_sq)
    model.add_member(mem1)
    model.add_member(mem2)
    model.add_member(mem3)

    # Supports
    sup1 = Support.fixed(n1.id)
    sup3 = Support.pinned(n3.id)
    sup4 = Support.roller_x(n4.id)
    model.add_support(sup1)
    model.add_support(sup3)
    model.add_support(sup4)

    # Loads
    load_nodal = NodalLoad(301, n2.id, fx=10e3, mz=-5e3, label="Corner Load")
    load_pt = MemberPointLoad(302, mem1.id, px=0, py=-15e3, position=2.5, label="Mid Span Pt")
    load_udl = MemberUDLoad(303, mem3.id, wx=0, wy=-2e3, label="Snow Load")
    model.add_load(load_nodal)
    model.add_load(load_pt)
    model.add_load(load_udl)
